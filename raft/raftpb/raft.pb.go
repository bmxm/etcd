...
// Raft 算法实现了一种复制状态机。
// 每个分布式的状态机中存储了一份包含命令序列的日志文件，这些文件通过复制的形式传播到其他节点中。
// 每个日志包含相同的命令，并且顺序也相同。状态机会按顺序执行这些命令并产生相同的状态，最终所有的状态机都将达到一个确定的状态。

// Raft 算法中每个节点会维护一份日志（Replicated Log），
// 复制日志中存储了按顺序排列的条目（Entry），
// 用户执行的每一个操作都会生成日志中的一个条目，
// 稍后这个条目会通过节点之间的交流复制到所有节点。

// 如果一个条目是被大多数节点认可的，那么这种条目被称为 Committed Entry,这也是节点唯一会执行的条目类型。
// 各个节点只要按顺序执行复制日志中的 Committed Entry，最终就会达到相同的状态。
// 这样，即便节点崩溃后苏醒，也可以快速恢复到和其他节点相同的状态。
