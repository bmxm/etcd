...
// Raft 中的节点有三种状态，
// Leader 领导者：负责数据修改，主动同步修改变更给 Follower（处理写请求、管理日志复制、不断发送心跳消息）
// Candidate 候选人: 集群中如果没有 Leader，那么进行选举模式，想其他节点发送请求投票（RequestVote）RPC 消息。
// Follower 跟随者：接收 Leader 推送的变更数据，收到心跳超时时，会主动站出来，推荐自己当候选人。
//
// 不过这里定义了 4 或 5 种 ？

// 不直接采用Basic Paxos 选主，是因为在Raft中不是所有的节点都能当选领导者，只有大多数节点中日志最完整的节点才能当选领导者。
...
//
// 2. 还约定如果一个节点接收到一个包含较小的任期编号值的请求，那么它会直接拒绝这个请求。
// 比如节点 C 的任期编号为 4，收到包含任期编号为 3 的请求投票 RPC 消息，那么它将拒绝这个消息。

// 在初始状态下，集群中所有的节点都是跟随者的状态，任期是0.
